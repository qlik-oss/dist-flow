    ...
    colors: {
      ...
      items: {
        ...
        colorMode: {
          ...
          show(data) {
            return !propertyResolver.getValue(data, 'boxplotDef.color.auto', true);
          },
        },
        boxColor: {
          ...
          show(data) {
            return (
              !propertyResolver.getValue(data, 'boxplotDef.color.auto', true) &&
              propertyResolver.getValue(data, 'boxplotDef.color.mode', 'primary') === 'primary'
            );
          },
          defaultValue() {
            const color = chartStyleUtils.getStyle(chartID, 'box.box', 'fill');
        ...
        attributeExpression: {
          ...
          show(data) {
            return (
              !propertyResolver.getValue(data, 'boxplotDef.color.auto', true) &&
              propertyResolver.getValue(data, 'boxplotDef.color.mode') === 'byExpression'
            );
          },
        },
        expressionIsColor: {
...
const dimensionAxis = {
  ...
  show(properties, handler, args) {
    const hasSecondDimension =
      propertyResolver.getValue(args.layout, 'boxplotDef.qHyperCube.qDimensionInfo.length') > 1;
...
const expressionItem = {
  ...
  items: {
    ...
    sortByElement: {
      ...
      show(properties) {
        return propertyResolver.getValue(properties, `${SORTING_REFS.SORT_CRITERIA}.sortByExpression`, 0) !== 0;
      },
      elements(args) {
    ...
    sortingItems: {
      ...
      items: {
        ...
        defaultSortMessage: {
          ...
          show(properties) {
            return (
              propertyResolver.getValue(properties, `${SORTING_REFS.SORT_CRITERIA}.sortByExpression`, 0) === 0 &&
              propertyResolver.getValue(properties, `${SORTING_REFS.SORT_CRITERIA}.sortByNumeric`, 0) === 0 &&
              propertyResolver.getValue(properties, `${SORTING_REFS.SORT_CRITERIA}.sortByAscii`, 0) === 0
            );
          },
        },
      },
      show(properties) {
        return hasMultipleDimensions(properties) && !propertyResolver.getValue(properties, SORTING_REFS.AUTO_SORT);
      },
    },
    noData: {
      component: 'no-data',
  ...
  return {
    get(getter, definition, args) {
      ...
      if (args.properties.qUndoExclude && (value === null || value === 'NaN' || value === '')) {
        const expression = getDefaultExpression(args.properties.qUndoExclude.box.qHyperCubeDef.qMeasures, elementName);
        return expression || '';
      }
      return getter(null) || '';
    },
...
function nameConvertFunctions(elementName) {
  return {
    ...
    set(value, setter, definition) {
      ...
      setter(definition.type, value, defaultValue);
    },
  };
}

function isNotFirstDimension(itemData, handler) {
  return propertyResolver.getValue(handler.properties, 'boxplotDef.qHyperCubeDef.qDimensions', [])[0] !== itemData;
}
  ...
  items: {
    ...
    sortOutliers: {
      ...
      show(data) {
        const mode = data.boxplotDef.calculations.mode;
        const calcParam = data.boxplotDef.calculations.parameters[mode];
        const showCond = !(!inAdvancedMode(data) && mode === boxplotUtils.BOXMODES.FRACTILES.value && calcParam === 0);
