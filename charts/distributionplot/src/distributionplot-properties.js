...
import propHcHelper from '../../../assets/client/utils/property-hypercube-helper';
import propertyLogicModule from '../../../assets/client/utils/default-property-logic';
import sortOrderBuilder from '../../../assets/client/property-panel/definition-builder/sort-order/sort-order';
...
const propertyLogic = propertyLogicModule.getLogicFor('hard', 'distributionplot');

function persistentColorsShowFunc(data) {
  return (
    !getValue(data, 'color.point.auto') &&
    getValue(data, 'color.point.mode') === 'byDimension' &&
    propHcHelper.getNumMeasures(data) <= 1
  );
}

// colors.items.colors.items.pointColor.items.colors.items.colorMode.options = ;
            ...
            colors: {
              ...
              items: {
                ...
                boxColor: {
                  ...
                  dualOutput: true,
                  defaultValue() {
                    const color = chartStyleUtils.getStyle(CONSTANTS.CHART_ID, 'box', 'fill');
                    const index = chartStyleUtils.Theme.currentPalette.indexOf(color);
                    return { index, color };
                  },
                },
                paletteColor: {
                  ...
                  defaultValue() {
                    const color = chartStyleUtils.Theme.dataColors.primaryColor;
                    const index = chartStyleUtils.Theme.currentPalette.indexOf(color);
                    return { index, color };
                  },
                },
                attributeExpression: {
                  ...
                  show(data) {
                    return !getValue(data, 'color.point.auto') && getValue(data, 'color.point.mode') === 'byExpression';
                  },
                },
                expressionIsColor: {
...
const general = {
  items: {
    showDisclaimer: {
      ...
      component: 'switch',
      show() {
        return isEnabled('SHOW_DISCLAIMER');
      },
      defaultValue: true,
      options: [
        {
    ...
    colors: {
      ...
      items: {
        ...
        baseColors: {
          ...
          items: {
            useBaseColor: {
              ...
              // show: propertyLogic.color.baseColor.show,
              show(data, handler) {
              ...
              // globalChange: function ( data ) {
              // TODO: This code only applies if we use mode === "byMultiple" which we will never support?
              // 	// Multicolor + dimension color is nonsensical
              // 	if ( getValue( data, "color.point.mode", "" ) === "byMultiple" && getValue( data, "color.point.useBaseColors", "" ) === "dimension" ) {
              // 		propertyResolver.setValue( data, "color.point.useBaseColors", "measure" );
              // 	}
              // },
              options(data, handler) {
                const baseOptions = [{ value: 'off', translation: 'Common.None' }];

                if (propertyLogic.color.baseColor.hasMeasureColor(handler)) {
                  baseOptions.push({
                    value: 'measure',
                    translation: 'Common.Measure',
                  });
                }
                if (propertyLogic.color.baseColor.hasDimensionColor(handler) && data.color.mode !== 'byMultiple') {
                  baseOptions.push({
                    value: 'dimension',
        ...
        paletteColor: {
          ...
          // 	var color = chartStyleUtils.Theme.dataColors.primaryColor,
          // 		index = chartStyleUtils.Theme.currentPalette.indexOf( color );
        ...
        colorSchemeDimension: {
          ...
          // TODO: Do we need to change how we fetch/generate schemes with Picasso charts and will that affect this code?
          // 	defaultValue: getValue( theme, "dataPalettes.0.propertyValue", "12" ), // TODO: New themes for Picasso charts?
          // 	items: propertyLogic.helpers.getDimensionSchemeOptions,
          show(data) {
            const show =
