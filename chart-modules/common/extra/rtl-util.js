...
const // left-to-right marker
  lrm = String.fromCharCode(8206); // right-to-left marker

const rlm = String.fromCharCode(8207);
let scrollImplementation;
let supportAuto;

function doesSupportDirAuto() {
  if (supportAuto === undefined) {
    detectScrollImplementationAndAutoSupport();
  }
  return supportAuto;
}
...
function getAutoOrAlternativeDirection(s) {
  if (supportAuto) {
    return 'auto';
  }
  return detectTextDirection(s);
}

function detectScrollImplementationAndAutoSupport() {
  const rtlTestDiv = document.createElement('div');
  const body = document.getElementsByTagName('body')[0];

  rtlTestDiv.setAttribute(
    'style',
    'position: absolute; width: 1px; height: 1px; top: -999px; overflow: scroll; white-space:nowrap;'
  );
  rtlTestDiv.setAttribute('dir', 'rtl');
  body.appendChild(rtlTestDiv);

  scrollImplementation = detectScrollImplementation(rtlTestDiv);
  supportAuto = detectAutoSupport(rtlTestDiv);

  body.removeChild(rtlTestDiv);
}

function detectScrollImplementation(rtlTestDiv) {
  rtlTestDiv.innerHTML = 'RTL-SCROLL-TEST';
  let scrollImpl = 'reverse';

  if (rtlTestDiv.scrollLeft > 0) {
    scrollImpl = 'default';
  } else {
    rtlTestDiv.scrollLeft = 1;
    if (rtlTestDiv.scrollLeft === 0) {
      scrollImpl = 'negative';
    }
  }
...
function detectAutoSupport(rtlTestDiv) {
  rtlTestDiv.innerHTML = 'لا أحد يحب الألم بذاته، يسعى ورائه أو يبت'; // arabic lorem ipsum

  let autoSup;

  switch (scrollImplementation) {
    case 'reverse':
      autoSup = false; // if the implementation is reverse (IE11,Edge), there is no way of detecting auto support, so we have to be safe and return false
      break;
    case 'default':
      autoSup = rtlTestDiv.scrollLeft > 0; // if auto is implemented then the scrollLeft will be > 0
      break;
    case 'negative':
      $(rtlTestDiv).scrollLeft(-1000); // try negative scrolling
      autoSup = rtlTestDiv.scrollLeft < 0; // if it worked, auto is implemented
      break;
    default:
      autoSup = false;
  }
...
function getScrollPos(isRtl, startScrollPos, delta) {
  delta = Number.isNaN(+delta) ? 0 : delta;
  if (isRtl && scrollImplementation === 'reverse') {
    return startScrollPos + delta;
  }
  return startScrollPos - delta;
}

export default {
  supportDirAuto: doesSupportDirAuto,
  detectTextDirection,
  getAutoOrAlternativeDirection,
  isLtrChar,
  isRtlChar,
  isNeutralChar,
  lrm,
